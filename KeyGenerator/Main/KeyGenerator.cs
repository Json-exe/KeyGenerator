...
public class KeyGenerator
{
    ...
    {
        if (AnsiConsole.Confirm("Do you want to change, how many keys you want to generate? Currently you want to generate " + key.Keys + " keys."))
        {
            key.Keys = AnsiConsole.Ask<int>("How many keys do you want to generate?");
        }
        ...
        for (var i = 0; i < key.Keys; i++)
        {
            ...
            foreach (var keyModule in key.KeyModules)
            {
                ...
                if (!keyPart.EndsWith(key.Separator) && key.KeyModules.Last() != keyModule)
                {
        ...
        if (AnsiConsole.Confirm("Do you want to save your keys to a file?"))
        {
            ...
            { 
                savePath = AnsiConsole.Ask<string>("Where do you want to save your keys? (Please enter a valid directory path!)");
            }
            SaveKeys(keyList, savePath);
        }

        if (!SystemHandler.Keys.Any(x => x.Name == key.Name) && AnsiConsole.Confirm("Do you want to save your Key Template for later use? You can easily load it at anytime!"))
        {
